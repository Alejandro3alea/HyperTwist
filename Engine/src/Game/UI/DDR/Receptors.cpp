...
void Receptors::Initialize()
{
    ...
    for (unsigned i = 0; i < 4; i++)
    {
        ...
        // @TODO
        mReceptorOn[i].mTexture = mReceptorOff[i].mTexture =
        ...
        mReceptorOn[i].transform.pos.x = mReceptorOff[i].transform.pos.x =
            noteInbetweenOffset * gGlobalVariables.mZoom * noteDefaultSize / 2.0f;
        mReceptorOn[i].transform.pos.y = mReceptorOff[i].transform.pos.y = -400.0f;
        mReceptorOn[i].transform.pos.z = mReceptorOff[i].transform.pos.z = 0.5f;
        mReceptorOn[i].transform.scale = mReceptorOff[i].transform.scale = {noteDefaultSize, noteDefaultSize, 1.0f};
