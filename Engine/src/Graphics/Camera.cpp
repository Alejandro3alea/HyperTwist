...
Camera::Camera() { mPos = glm::vec3(0.f, 0.0f, 0.f); }

glm::mat4x4 Camera::GetViewMtx() { return glm::translate(glm::mat4(1.0f), -mPos); }
...
void Camera::Move(const glm::vec2& vec) { mPos += glm::vec3(vec.x, vec.y, 0.0f); }

void Camera::Move(const glm::vec3& vec) { mPos += vec; }
