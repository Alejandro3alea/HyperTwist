...
Font::Font(const std::string& path)
{
    ...
    else
    {
        ...
        for (unsigned char c = 0; c < 128; c++)
        {
            ...
            glTexSubImage3D(GL_TEXTURE_2D_ARRAY, 0, 0, 0, int(c), face->glyph->bitmap.width, face->glyph->bitmap.rows,
            ...
            Character character = {uint32_t(c), glm::ivec2(face->glyph->bitmap.width, face->glyph->bitmap.rows),
...
void Font::BindFontTexture() { glBindTexture(GL_TEXTURE_2D_ARRAY, mTextureArrayIndex); }