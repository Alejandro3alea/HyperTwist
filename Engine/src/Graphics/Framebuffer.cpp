...
void Framebuffer::BindFramebuffer() { glBindFramebuffer(GL_FRAMEBUFFER, mFbID); }
...
GLuint ColorBuffer::Construct(const size_t bufferIdx) const
{
    ...
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0 + static_cast<GLenum>(bufferIdx), GL_TEXTURE_2D, texID,
                           0);
