    ...
    : mSize(winWidth, winHeight)
{
    ...
    SDL_GL_MakeCurrent(mWindow_SDL, mWinContext);
...
void WindowManager::Initialize(const std::string& windowName)
{
    ...
    InputMgr->mOnWindowResize.Add([this](const uint32_t width, const uint32_t height)
                                  { mCurrentWindow->Resize(glm::uvec2(width, height)); });
}
