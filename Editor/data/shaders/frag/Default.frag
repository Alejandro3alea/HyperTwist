...
{		
    ...
    float roughness = texture(uMaterial.Metallic, fs_in.TexUV).r;
    float ao        = texture(uMaterial.Metallic, fs_in.TexUV).r;
    ...
    for(int i = 0; i < 4; ++i) 
    {
        ...
        vec3 L = normalize(uLight[i].Position - fs_in.FragPos);
        vec3 H = normalize(V + L);
        float distance = length(uLight[i].Position - fs_in.FragPos);
