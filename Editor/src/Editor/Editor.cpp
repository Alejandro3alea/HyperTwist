    ...
    if (mSong && mSong->get()->IsPlaying())
    {
        ...
        if (!mPauseTimer.IsActive())
        {
            ...
            if ((NextNoteL && !NextNoteR) ||
                (NextNoteL && NextNoteR &&
                 Editor->GetNextNote(mCurrMeasure)->mPos != Editor->GetNextNote(nextStep)->mPos))
            {
                mNoteTick->get()->Play();
            }
        ...
        if (ImGui::BeginMenu("File")) // MainMenu block
        {
            ...
            if (ImGui::MenuItem("Open"))
            {
                ...
                if (languageID == MAKELANGID(LANG_SPANISH, SUBLANG_DEFAULT))
                    ofn.lpstrFilter =
                        L"Archivos compatibles (*.sm, *.ssc, *.ogg, *.mp3, "
                        L"*.wav)\0*.sm;*.ssc;*.ogg;*.mp3;*.wav\0Stepmania/ITG (*.sm)\0*.sm\0Stepmania 5 "
                        L"(*.ssc)\0*.ssc\0Archivo Ogg Vorbis (*.ogg)\0*.ogg\0Archivo MP3 (*.mp3)\0*.mp3\0Archivo "
                        L"Waveform (*.wav)\0*.wav\0Todos los archivos (*.*)\0*.*\0";
                else
                    ofn.lpstrFilter = L"Compatible Files (*.sm, *.ssc, *.ogg, *.mp3, "
                                      L"*.wav)\0*.sm;*.ssc;*.ogg;*.mp3;*.wav\0Stepmania/ITG (*.sm)\0*.sm\0Stepmania 5 "
                                      L"(*.ssc)\0*.ssc\0Ogg Vorbis File (*.ogg)\0*.ogg\0MP3 File "
                                      L"(*.mp3)\0*.mp3\0Waveform File (*.wav)\0*.wav\0All Files (*.*)\0*.*\0";
...
void ImGuiEditor::SetImGuiStyle()
{
    ...
    style.Colors[ImGuiCol_TabUnfocusedActive] =
        ImLerp(style.Colors[ImGuiCol_TabActive], style.Colors[ImGuiCol_TitleBg], 0.40f);
    ...
    if (ImGui::Begin("Song Properties") && mSong != nullptr)
    {
        ...
        if (Texture* banner = song->GetBanner())
        {
            ...
            ImGui::Image(reinterpret_cast<ImTextureID>(texID),
                         {ImGui::GetWindowSize().x, ImGui::GetWindowSize().x / 3.0f}, {0, 1}, {1, 0});
        }
