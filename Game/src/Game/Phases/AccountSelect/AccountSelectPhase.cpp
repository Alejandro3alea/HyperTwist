...
void AccountSelectPhase::OnTick(const float dt)
{
    ...
    {
        SetupAccountIfFree(PLAYER_1_IDX, TEST_ACCOUNTS_NFC_PLAYER_1);
    }
    if (InputMgr->isKeyPressed(SDL_SCANCODE_KP_MINUS))
    {
        SetupAccountIfFree(PLAYER_2_IDX, TEST_ACCOUNTS_NFC_PLAYER_2);
    }
...
{
    std::array<glm::vec4, 3> transitionColors = {Color::FromInt(250, 252, 254), Color::FromInt(43, 237, 255),
...
void AccountSelectPhase::UpdateIntroTransition(const float dt)
{
    ...
    for (unsigned i = 0; i < mTransitionRenderers.size(); i++)
    {
        mTransitionRenderers[i]->transform.pos =
            Math::Lerp(transitionPosStart[i], transitionPosEnd[i], mTransitionTimer / maxTransitionTime);
    }
}
...
void AccountSelectPhase::OnTransitionEnd()
{
    ...
    for (unsigned i = 0; i < mAccountWindows.size(); i++)
    {
        if (AccountMgr->IsPlayerSlotOccupied(i))
        {
            ...
            mAccountWindows[i]->mOnAccountWindowClosed.Add([this](const uint32_t playerID)
                                                           { OnWindowClosed(playerID); });
        }
    }
...
void AccountSelectPhase::SetupAccountIfFree(const uint32_t playerID, const std::string& playerNfc)
{
    if (AccountMgr->IsPlayerSlotFree(playerID))
    {
        ...
        mAccountWindows[playerID]->mOnAccountWindowClosed.Add([this](const uint32_t playerID)
                                                              { OnWindowClosed(playerID); });
    }
